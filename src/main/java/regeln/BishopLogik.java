 ...
 * @author roman
 */
public class BishopLogik extends Bewegungslogik implements Regeln {
    
    protected void mache_Schachzug_fuer_Bishop(Spielrunde runde) {
        Bishop bishop = (Bishop) runde.getAltes_ausgewähltes_Schachfeld().getSchachfigur().getSpielfigur();
        System.out.println(bishop.toString());
        if (bishop.getSpieler().getSpielerKennzeichen().equals("Spieler1")) {
